import pytest
import pandas
from dataprocessing.common import round_half_up, getandprocessJSONdataENTSOG, get_json_data_entsog, get_excel_data, \
    executeRequest
...
def test_getandprocessJSONdataENTSOG(check, result):
    assert(type(getandprocessJSONdataENTSOG(check)) is result)

@pytest.mark.parametrize("check, result",
                         [("{'meta': {'limit': -1}, 'operationalData':[{'indicator': 'PF', "
                           "'periodFrom': '2021-08-01T06:00:00+02:00', 'pointLabel': 'VIP', 'value': 131450000}, "
                           "{'indicator': 'GCV', 'periodFrom': '2021-08-01T06:00:00+02:00', "
                           "'pointLabel': 'VIP', 'value': 11.44}]}", pandas.DataFrame),
                          ({"'meta': {'limit': -1}, 'operationalData': [{'indicator': 'PF', "
...
@pytest.mark.parametrize("check, result", [('http://localhost', None),
                         ('https://dog.ceo/api/breed/hound/list', b'{"message":["afghan","basset","blood","english",'
                                                                  b'"ibizan","plott","walker"],"status":"success"}')])
def test_execute_request(check, result):
    assert(executeRequest(check) == result)

@pytest.mark.parametrize("check, result", [('https://transparency.entsog.eu/api/v1/operationalData.csv?forceDownload=true&'
